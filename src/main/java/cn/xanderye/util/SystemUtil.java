...
public class SystemUtil {
    ...
    public static void execVcsi(Progress progress, String filePath, String outputPath, String width, String row, String col) {
        ...
        cmd = cmd.replace("${videoPath}", filePath)
                ...
                .replace("${row}", row)
                .replace("${col}", col)
                .replace("${fileName}", progress.getName());
        String[] cmds = cmd.split(BREAK);
        Process process = null;
        try {
            process = new ProcessBuilder(cmds).redirectErrorStream(true).start();
            InputStream is = process.getInputStream();
            BufferedReader buffer = new BufferedReader(new InputStreamReader(is, Charset.defaultCharset()));
            String line;
            while ((line = buffer.readLine()) != null) {
                if (line.contains("Sampling...")) {
                    String data = line.replace("Sampling... ", "");
                    String[] ss = data.split("/");
                    int a = Integer.parseInt(ss[0]);
                    int b = Integer.parseInt(ss[1]);
                    System.out.println(a + " " + b);
                    progress.setPercent(a * 100 / b);
                }
            }
            is.close();
        } catch (Exception e) {
